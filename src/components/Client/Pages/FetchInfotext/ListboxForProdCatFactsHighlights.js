...
export default function ListboxForProdCatFactsHighlights({
	arr,
	infotextDe,
	infotextFr,
	infotextEn,
	...
	async function updateproductcategory() {
		...
		const data = await fetch(
			`/api/supabase/update-facts-and-highlights-productcategory${str}`
		);
		const res = await data.json();
		const info =
			res.error == null
				? "done"
				: "ein Fehler ist aufgetreten: " + JSON.stringify(res.error);
